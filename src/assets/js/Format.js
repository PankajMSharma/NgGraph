...
TextFormatPanel.prototype.addFont = function(container)
{
	...
	var dirSelect = document.createElement('input');
...
// Username
var stylePanel6 = stylePanel.cloneNode(false);
stylePanel6.style.marginLeft = '0px';
stylePanel6.style.paddingTop = '4px';
stylePanel6.style.paddingBottom = '4px';
stylePanel6.style.fontWeight = 'normal';

mxUtils.write(stylePanel6, mxResources.get('userName'));
...
dirInput.style.position = 'absolute';
dirInput.style.right = '20px';
dirInput.style.width = '97px';
dirInput.style.marginTop = '-2px';

// NOTE: For automatic we use the value null since automatic
// requires the text to be non formatted and non-wrapped
var dirSetForUserName = {'automatic': null,
		'leftToRight': mxConstants.TEXT_DIRECTION_LTR,
		'rightToLeft': mxConstants.TEXT_DIRECTION_RTL};
...
for (var i = 0; i < dirs.length; i++)
{
	...
	dirOption.setAttribute('value', dirs[i]);
	mxUtils.write(dirOption, mxResources.get(dirs[i]));
	dirInput.appendChild(dirOption);
}

stylePanel6.appendChild(dirInput);

// URL
var stylePanel7 = stylePanel.cloneNode(false);
stylePanel7.style.marginLeft = '0px';
stylePanel7.style.paddingTop = '4px';
stylePanel7.style.paddingBottom = '4px';
stylePanel7.style.fontWeight = 'normal';

mxUtils.write(stylePanel7, mxResources.get('url'));

// Adds writing direction options
// LATER: Handle reselect of same option in all selects (change event
// is not fired for same option so have opened state on click) and
// handle multiple different styles for current selection
var dirUrl = document.createElement('input');
dirUrl.style.position = 'absolute';
dirUrl.style.right = '20px';
dirUrl.style.width = '97px';
dirUrl.style.marginTop = '-2px';

// NOTE: For automatic we use the value null since automatic
// requires the text to be non formatted and non-wrapped
var dirSetForUrl = {'automatic': null,
		'leftToRight': mxConstants.TEXT_DIRECTION_LTR,
		'rightToLeft': mxConstants.TEXT_DIRECTION_RTL};

stylePanel7.appendChild(dirUrl);

// Password
var stylePanel8 = stylePanel.cloneNode(false);
stylePanel8.style.marginLeft = '0px';
stylePanel8.style.paddingTop = '4px';
stylePanel8.style.paddingBottom = '4px';
stylePanel8.style.fontWeight = 'normal';

mxUtils.write(stylePanel8, mxResources.get('password'));

// Adds writing direction options
// LATER: Handle reselect of same option in all selects (change event
// is not fired for same option so have opened state on click) and
// handle multiple different styles for current selection
var dirPasswd = document.createElement('input');
dirPasswd.setAttribute('type', 'password');
dirPasswd.style.position = 'absolute';
dirPasswd.style.right = '20px';
dirPasswd.style.width = '97px';
dirPasswd.style.marginTop = '-2px';

// NOTE: For automatic we use the value null since automatic
// requires the text to be non formatted and non-wrapped
var dirSetForPasswd = {'automatic': null,
		'leftToRight': mxConstants.TEXT_DIRECTION_LTR,
		'rightToLeft': mxConstants.TEXT_DIRECTION_RTL};

stylePanel8.appendChild(dirPasswd);
	...
	if (!graph.isEditing())
	{
		...
		container.appendChild(stylePanel6);
		container.appendChild(stylePanel8);
		container.appendChild(stylePanel7);
		...
		mxEvent.addListener(dirInput, 'change', function(evt)
		{
			graph.setCellStyles(mxConstants.STYLE_TEXT_DIRECTION, dirSetForUserName[dirInput.value], graph.getSelectionCells());
			mxEvent.consume(evt);
		});
		mxEvent.addListener(dirUrl, 'change', function(evt)
		{
			graph.setCellStyles(mxConstants.STYLE_TEXT_DIRECTION, dirSetForUrl[dirUrl.value], graph.getSelectionCells());
			mxEvent.consume(evt);
		});
		mxEvent.addListener(dirPasswd, 'change', function(evt)
		{
			graph.setCellStyles(mxConstants.STYLE_TEXT_DIRECTION, dirSetForPasswd[dirPasswd.value], graph.getSelectionCells());
			mxEvent.consume(evt);
		});
	}
